...
class Popup {
	...
	eventsPopup() {
      ...
      let closeCondition = !e.target.closest(`.${this.options.classes.popupContent}`) && !e.target.closest('.lg-container');
			if (buttonClose || closeCondition && this.isOpen) {
				e.preventDefault();
				await this.close();
				return;
			}
    ...
    return new Promise(async resolve => {
      ...
      if (bodyLockStatus) {
        ...
        if (this.targetOpen.element) {
          ...
          setTimeout(() => {
            this.targetOpen.element.classList.add('_open');
          }, 310);
    ...
    return new Promise(resolve => {
      ...
      this.previousOpen.element.classList.remove('_open');
